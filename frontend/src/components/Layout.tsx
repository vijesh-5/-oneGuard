...
// Removed LayoutProps interface as children is no longer used directly

const Layout: React.FC = () => { // Removed Props type
    const { logout, user, toggleMode } = useAuth();
    ...
    const handleLogout = () => {
        logout();
        navigate('/login'); // Should be handled by AuthContext but explicit here too
    };
                    ...
                    <div className="p-4 border-t border-slate-800 bg-slate-900/50">
                        <div className="flex items-center gap-3 p-3 rounded-lg hover:bg-slate-800 transition-colors cursor-pointer group">
                            ...
                            <div className="flex-1 min-w-0">
                                ...
                                <p className="text-xs text-slate-500 truncate capitalize">{user?.mode} Account</p>
                            </div>
            ...
            <div className="flex-1 flex flex-col min-w-0 lg:pl-72 transition-all duration-300">
                ...
                <header className="h-20 bg-slate-900/50 backdrop-blur-md border-b border-slate-800 flex items-center justify-between px-8 sticky top-0 z-20">
                    ...
                    <div className="flex items-center gap-4 ml-auto">
                        ...
                        <button
                            onClick={toggleMode}
                            className={`px-4 py-2 rounded-lg text-sm font-bold transition-all ${user?.mode === 'business'
                                ? 'bg-cyan-500/10 text-cyan-400 hover:bg-cyan-500/20'
                                : 'bg-purple-500/10 text-purple-400 hover:bg-purple-500/20'
                                }`}
                        >
                            {user?.mode === 'business' ? 'Business Mode' : 'Personal Mode'}
                        </button>
                        ...
                        <button className="p-2 text-slate-400 hover:text-cyan-400 transition-colors rounded-lg hover:bg-slate-800/50">
